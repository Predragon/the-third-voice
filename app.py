    ...
    if submit_button and message:
        with st.spinner("🌟 Channeling wisdom and love..."):
            try:
                ...
                # Display response with healing focus
                st.markdown(f"""
                ...
                save_message(contact_key, mode, message, reply, "healing", st.session_state.current_emotional_state, "LLaMA 3.2 3B", healing_score)
...
def render_history():
    ...
    for entry in reversed(history[-10:]):  # Show last 10 entries
        ...
        with st.expander(f"{icon} {entry['time']} • {entry['type'].title()} • Feeling: {entry.get('emotional_state', 'calm')}"):
            ...
            st.markdown(f'<div class="healing-message">{entry["result"]}</div>', unsafe_allow_html=True)
