...
class AuthManager:
    ...
    def restore_session(self) -> bool:
        ...
        if state_manager.get('logged_out_intentionally', False):
            logger.info("Skipping session restoration due to intentional logout")
            return False
        try:
    ...
    def sign_in(self, email: str, password: str) -> bool:
        ...
        try:
            ...
            if response.user:
                ...
                state_manager.set('logged_out_intentionally', False)  # Reset flag
                # Load user data
                from .data_manager import data_manager
                contacts = data_manager.load_contacts_and_history()
                state_manager.set_contacts(contacts)
    ...
    def sign_out(self) -> bool:
        ...
        try:
            ...
            if response is None:
                logger.warning("Supabase sign_out returned None, assuming logout successful but clearing state manually")
                state_manager.clear_authentication()
                state_manager.set('logged_out_intentionally', True)
                from .data_manager import data_manager
                data_manager.clear_cache()
                st.info("You have been logged out.")
                return True
            if not response.error:
                logger.info("Supabase sign_out successful")
                state_manager.clear_authentication()
                state_manager.set('logged_out_intentionally', True)
                from .data_manager import data_manager
                data_manager.clear_cache()
                ...
                logger.error(f"Supabase sign_out error: {response.error.message}")
                state_manager.set_error(f"Logout failed: {response.error.message}")
                return False
        except Exception as e:
            logger.error(f"Unexpected error during sign_out: {str(e)}")
