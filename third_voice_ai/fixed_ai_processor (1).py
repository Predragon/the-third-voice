...
class AIProcessor:
    ...
    def set_managers(self, auth_manager, data_manager):
        """Set the auth and data managers to avoid circular imports"""
    ...
    def process_message(self, contact_name: str, message: str, context: str, history: List[Dict] = None, is_incoming: bool = False) -> Dict[str, Any]:
        ...
        # Check cache first if we have data_manager
        if self.data_manager:
            ...
            if contact_id:
                cached_response = self.data_manager.get_cached_ai_response(contact_id, message_hash)
                if cached_response:
        ...
        if result["success"]:
            ...
            # Cache the response if we have data_manager
            if self.data_manager:
                contact_data = state_manager.get_contact_data(contact_name)
                contact_id = contact_data.get("id")
                if contact_id:
                    message_hash = utils.create_message_hash(message, context)
                    self.data_manager.cache_ai_response(
