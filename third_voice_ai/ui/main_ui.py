...
class MainUI:
    ...
    @staticmethod
    def _show_analytics_section(data_manager) -> None:
        """Show analytics section if enabled"""
        try:
            stats = data_manager.get_user_stats()
            st.subheader("Your Stats")
            col1, col2, col3 = create_three_column_layout()
            with col1:
                create_metric_card("Total Contacts", stats.get('contact_count', 0))
            with col2:
                create_metric_card("Total Messages", stats.get('message_count', 0))
            with col3:
                create_metric_card("Avg Healing Score", f"{stats.get('avg_healing_score', 0):.1f}")
    ...
    @staticmethod
    def _show_conversation_navigation(state_manager, contact_id: str, active_contact: str, context: str) -> None:
        """Show conversation navigation buttons"""
        back_col, edit_col, _ = create_three_column_layout()
        with back_col:
        ...
        with edit_col:
            if show_navigation_button("✏️ Edit", "edit_current_contact"):
