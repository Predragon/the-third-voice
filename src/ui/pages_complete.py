...
Updated for TRUE Supabase-centric authentication
...
class AuthenticationUI:
    """Authentication user interface - True Supabase-centric approach"""
    
    def __init__(self, auth_manager):
        self.auth_manager = auth_manager
    
    def run(self) -> bool:
        """Run authentication flow with multiple options
        
        CRITICAL: Always check Supabase authentication, never trust session state
        """
        
        # CRITICAL: Check Supabase authentication first
        ...
        **Transform difficult conversations into healing dialogues**
        ...
        tab1, tab2, tab3 = st.tabs(["🔐 Google Login", "📧 Email Login", "📝 Create Account"])
        
        with tab1:
            authenticated = self._render_google_login()
            if authenticated:
                return True
        
        with tab2:
            authenticated = self._render_email_login()
            if authenticated:
                return True
    ...
    def _handle_oauth_callback(self) -> bool:
        ...
        Check if we're returning from OAuth and need to process the result
        """
        try:
            query_params = st.query_params
            oauth_params = ['access_token', 'refresh_token', 'code', 'type']
            
            if any(param in query_params for param in oauth_params):
                st.info("🔄 Processing sign in...")
                
                # Give Supabase time to process OAuth
                import time
                time.sleep(1.0)
                
                # Check if authentication succeeded
                if self.auth_manager.is_authenticated():
                    user_email = self.auth_manager.get_current_user_email()
    ...
    def _render_google_login(self) -> bool:
        """Render Google OAuth login using streamlit-supabase-auth"""
        st.subheader("🔐 Quick Sign In with Google")
        st.markdown("*Fastest way to get started - no password needed!*")
        
        try:
            # Use streamlit-supabase-auth for Google login
            session = self.auth_manager.render_login_form()
            
            if session:
                # Don't trust the session immediately - verify with Supabase
                if self.auth_manager.is_authenticated():
                    st.success("✅ Successfully signed in with Google!")
                    return True
                else:
                    st.warning("🔄 Signing you in... please wait")
                    return False
                    
        except Exception as e:
            st.error(f"Google login error: {str(e)}")
            st.info("💡 Try the Email Login tab if Google isn't working")
        
        return False
    
    def _render_email_login(self) -> bool:
        """Render traditional email/password login"""
        ...
        with st.form("email_sign_in_form"):
                    ...
                    if email and password:
                        ...
                        if success:
                            ...
                            if self.auth_manager.is_authenticated():
                                ...
                                st.warning("🔄 Signing you in... please refresh")
                                return False
                        else:
                            st.error(message)
                    else:
            ...
            with col2:
                if st.form_submit_button("Forgot Password?", use_container_width=True):
                    st.info("💡 Try signing up again or contact support at hello@thethirdvoice.ai")
...
class OnboardingFlow:
    """First-time user onboarding - updated for True Supabase-centric auth"""
...
class Dashboard:
    """Main dashboard after onboarding - updated for True Supabase-centric auth"""
...
class AdminDashboard:
    """Admin dashboard for viewing feedback and analytics - updated for True Supabase-centric auth"""
