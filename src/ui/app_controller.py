...
Enhanced with demo account support and session persistence
"""
...
from ..auth.demo_manager import DemoManager  # Demo account support
...
class ThirdVoiceApp:
    ...
    def run(self):
        ...
        # Authentication check (real users)
        if not DemoManager.is_demo() and not self.auth_ui.run():
            return
        
        # Get current user
        user_id = getattr(st.session_state, "user", None).id if DemoManager.is_demo() else self.auth_manager.get_current_user_id()
        if not user_id:
