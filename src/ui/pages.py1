...
Enhanced with demo authentication and demo-aware features
"""
...
class AuthenticationUI:
    """Enhanced Authentication user interface with demo support"""
...
class Dashboard:
    """Main dashboard after onboarding - Enhanced with demo-aware functionality"""
    ...
    def _render_add_first_contact(self, user_id: str, auth_manager):
        ...
        # Use demo-aware header
        UIComponents.render_demo_header(auth_manager)
        ...
        # Demo feature callout for new contacts
        if auth_manager.is_demo_user():
            UIComponents.render_demo_feature_callout("first_contact")
            ...
            if st.form_submit_button("Add Contact", use_container_width=True, type="primary"):
                if name:
                    ...
                    if contact:
                        st.session_state.selected_contact = contact
                        if auth_manager.is_demo_user():
                            st.success(f"Added {name} to your demo session! 🎭")
                        else:
    ...
    def _render_main_interface(self, user_id: str, contacts: List[Contact], auth_manager):
        """Render main interface with contacts and conversation (enhanced for demo)"""
        
        # Use demo-aware header
        UIComponents.render_demo_header(auth_manager)
        
        # Also render logout in sidebar for desktop users (demo-aware)
        if auth_manager.is_demo_user():
            UIComponents.render_logout_sidebar(auth_manager)
            self._render_demo_enhancements(auth_manager)
        else:
            UIComponents.render_logout_sidebar(auth_manager)
        
        # Show demo feature callout for contacts
        if auth_manager.is_demo_user():
            UIComponents.render_demo_feature_callout("contacts")
        ...
        self._render_conversation_interface(user_id, selected_contact, auth_manager)
    ...
    def _render_conversation_interface(self, user_id: str, contact: Contact, auth_manager):
        """Render the main conversation interface (enhanced for demo)"""
        
        # Contact header
        context_enum = RelationshipContext(contact.context)
        UIComponents.render_contact_header(contact, context_enum)
        
        # Demo feature callout for conversations
        if auth_manager.is_demo_user():
            UIComponents.render_demo_feature_callout("conversations")
        ...
        if st.button("🎙️ Get Third Voice Help", use_container_width=True, type="primary", disabled=not message):
            self._process_message(user_id, contact, message, message_type, auth_manager)
        
        # Show conversation history
        self._render_conversation_history(user_id, contact, auth_manager)
    
    def _process_message(self, user_id: str, contact: Contact, message: str, message_type: str, auth_manager):
        """Process message with AI (enhanced for demo)"""
        ...
        if success:
            if auth_manager.is_demo_user():
                st.success("💾 Demo conversation saved for this session!")
            else:
                st.success("💾 Conversation saved!")
        
        # Show demo upgrade prompt after successful interaction
        if auth_manager.is_demo_user():
            messages_count = len(self.db._get_demo_user_data(user_id, 'messages'))
            if messages_count >= 2:  # After second message
                UIComponents.render_demo_upgrade_banner()
        ...
        if feedback_result:
            ...
            if self.db.save_feedback(user_id, rating, feedback_text, "conversation"):
                if auth_manager.is_demo_user():
                    st.success("Thank you for your demo feedback! 🎭")
                else:
                    st.success("Thank you for your feedback!")
                st.rerun()
    
    def _render_conversation_history(self, user_id: str, contact: Contact, auth_manager):
        """Render conversation history for the selected contact (enhanced for demo)"""
        ...
        if messages:
            ...
            # Demo callout for history
            if auth_manager.is_demo_user():
                UIComponents.render_demo_feature_callout("history")
            
            UIComponents.render_message_history(messages)
    
    def _render_demo_enhancements(self, auth_manager):
        """Add demo-specific UI enhancements"""
        if not auth_manager.is_demo_user():
            return
        
        user_id = auth_manager.get_current_user_id()
        
        # Show upgrade banner periodically
        demo_messages = self.db._get_demo_user_data(user_id, 'messages')
        if len(demo_messages) >= 3:
            UIComponents.render_demo_upgrade_banner()
        
        # Add demo data notice in sidebar
        with st.sidebar:
            UIComponents.render_demo_data_notice()
            
            # Show demo stats
            st.markdown("### 🎭 Demo Session")
            
            demo_contacts = len(self.db._get_demo_user_data(user_id, 'contacts'))
            demo_messages_count = len(demo_messages)
            
            st.metric("Contacts Created", demo_contacts)
            st.metric("Messages Processed", demo_messages_count)
            ...
            if time_remaining and time_remaining != "Expired":
                ...
                st.error("⏰ Demo session expired!")
                st.info("Sign up for free to continue!")
            
            st.markdown("---")
            UIComponents.render_demo_cta_buttons()
