...
class AIEngine:
    ...
    def __init__(self):
        ...
        self.models = [
            ...
            },
            {
                ...
                "name": "Llama 3.2 3B",
                "note": "Meta's instruction model"
            },
            {
                "id": "qwen/qwen-2.5-7b-instruct:free",
                "name": "Qwen 2.5 7B", 
                "note": "Alibaba's instruction model"
            },
            {
                "id": "google/gemma-2-9b-it:free",
                "name": "Gemma 2 9B",
                "note": "Google's instruction model"
            }
        ]
    ...
    def _get_model_display_name(self, model_id: str) -> str:
        ...
        for model in self.models:
            if model["id"] == model_id:
                return model["name"]
        return model_id.split("/")[-1].replace(":free", "")  # Fallback
    ...
    def _try_model(self, model_info: dict, system_prompt: str, user_prompt: str) -> dict:
        ...
        print(f"❌ Model {model_info['name']} failed: {response.text[:200] if response.text else 'No response'}")
        return None
            ...
            for user_prompt in user_prompts:
                try:
                    ...
                    if result:
                        ...
                        # Success! Return the result with model info
                        return AIResponse(
