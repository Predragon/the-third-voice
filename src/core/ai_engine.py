...
class RelationshipContext(Enum):
    ...
    def _missing_(cls, value):
        """Handle missing values by trying common variations"""
        if not isinstance(value, str):
            return None
            
        value_lower = value.lower()
        
        # Handle common variations and map them to database values
        variations = {
            'friends': cls.FRIEND,           # maps to 'friend'
            'co-parenting': cls.COPARENTING, # maps to 'coparenting'  
            'coparent': cls.COPARENTING,     # maps to 'coparenting'
            'work': cls.WORKPLACE,           # maps to 'workplace'
            'professional': cls.WORKPLACE,   # maps to 'workplace'
            'families': cls.FAMILY,          # maps to 'family'
        }
        
        return variations.get(value_lower)
...
class AIEngine:
    ...
    def process_message(self, message: str, contact_context: str, message_type: str,
        ...
        # Fixed the f-string syntax error by properly handling the multi-line string
        if message_type == MessageType.TRANSFORM.value:
            user_message = (
                f"{message_type.upper()} MODE: "
                f"I want to say something to my {context_desc}: \n\n"
                f"Message: \"{message}\"\n\n"
                "Please rewrite my message to be more loving and likely to be well-received"
            )
        else:
            user_message = (
                f"{message_type.upper()} MODE: "
                f"I received this message from my {context_desc}: \n\n"
                f"Message: \"{message}\"\n\n"
                "Please analyze the emotions and needs behind the message and suggest a loving, empathetic response. "
                "Provide a detailed explanation of the sender's subtext and needs."
            )
