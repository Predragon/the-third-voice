...
class AuthManager:
    ...
    def sign_in_with_google(self):
        """Initiate Google Sign-In with proper OAuth flow"""
        try:
            print("Debug: Initiating Google OAuth")
            
            # Get current app URL - this will work in both local and deployed environments
            current_url = st.get_option("browser.serverAddress") or "http://localhost:8501"
            if st.get_option("browser.serverPort"):
                port = st.get_option("browser.serverPort")
                if port != 80 and port != 443:
                    current_url = f"{current_url}:{port}"
            
            # For Streamlit Cloud, use the proper URL
    ...
    def handle_google_callback(self):
        ...
        if "code" in query_params:
            ...
            try:
                # Method 1: Try exchange_code_for_session (newer method)
                print("Debug: Attempting code exchange (Method 1)")
                ...
                try:
                    print("Debug: Attempting alternative method")
                    # Some Supabase client versions use different methods
                    session_response = self.supabase.auth.get_session_from_url({
                        "url": f"?code={auth_code}&token_type=bearer"
                    })
                    ...
                    if session_response and hasattr(session_response, 'user') and session_response.user:
                        ...
                        print(f"✅ User authenticated via Method 2: {session_response.user.email}")
                        st.query_params.clear()
                        st.rerun()
                        return session_response.user
                        
                except Exception as e2:
                    print(f"Debug: Method 2 also failed: {str(e2)}")
                    st.error(f"OAuth authentication failed: {str(e)}")
        
        # Check for access token (sometimes returned directly)
