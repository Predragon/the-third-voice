...
def show_conversation():
    ...
    if last_response:
        with st.container():
            ...
            with col_score:
                if last_response["healing_score"] >= 8:
                    st.success(f"✨ Healing Score: {last_response['healing_score']}/10")
                elif last_response["healing_score"] >= 6:
                    st.info(f"💡 Healing Score: {last_response['healing_score']}/10")
                else:
                    st.warning(f"🔧 Healing Score: {last_response['healing_score']}/10")
